...
def grad(sizes,x,u,pi,t,Q0,restrictions):
    ...
    for i in range(q+1):
        ...
        # Versão Muniz (Heun)
#        for k in range(N-1):
#            derk = -fx[k,:] + phixInv[k,:,:].dot(auxLam[k-1,:])
#            aux = auxLam[k,:] + dt*derk
#            auxLam[k+1,:] = auxLam[k,:] + 0.5*dt*(derk - fx[k+1,:] + phixInv[k+1,:,:].dot(aux))
...
#        plotSol(sizes,t,x,u,pi,constants,restrictions,optPlot)
    ...
    MaxIterRest = 100 #10000
    histP = numpy.zeros(MaxIterRest+1)
    histPint = histP.copy()
    histPpsi = histP.copy()
